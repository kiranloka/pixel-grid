      ...
      >
        {items.map((item, idx) => (
          <li
            ...
            key={item.name ? item.name + idx : idx}
          >
            {item.logo ? (
              <div className="flex items-center justify-center h-full min-h-[80px] px-8 opacity-70 hover:opacity-100 transition-opacity">
                {item.logo}
              </div>
            ) : item.image ? (
              <div className="flex items-center justify-center h-full min-h-[80px] px-8">
                <img
                  src={item.image}
                  alt={item.name || "Logo"}
